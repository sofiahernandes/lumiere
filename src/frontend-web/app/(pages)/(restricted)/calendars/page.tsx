  ...
  return (
    <section className="grid grid-cols-4 gap-4 md:grid-cols-12 pb-20">
      ...
      <div className="col-span-4 md:col-span-12">
        <h2 className="text-xl font-bold py-4">Selecione o Paciente</h2>
          ...
          <>
                ...
                <button
                  ...
                  className={`py-2 px-4 rounded-md border transition-all ${
                    selectedDay === day
                      ...
                      : 'bg-white/30 border-neutral-300 shadow-lg hover:bg-white'
                  }`}
                >
                  {day}
                </button>
            ...
            {tempExercises.length > 0 && (
              <div className="flex flex-col mt-6 p-4 border border-dashed border-blue/30 rounded-md bg-blue/5">
                ...
                {tempExercises.map((ex, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center p-3 mb-2 rounded border border-neutral-300 bg-white/30"
                  >
                    <span>
                    ...
                    <button
                      ...
                      className="p-2 hover:bg-salmon bg-salmon/50 rounded-full"
                    >
                      <Image
                        src="/lixo.png"
                        alt="Deletar"
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {exerciseSessions
                ...
                .map((es) => {
                  ...
                  // Verifica se o paciente sentiu dor
                  const hasPain = es.feelPain === true;

                  return (
                    <article
                      key={es.exerciseSession_ID}
                      className={`rounded-md border p-4 transition-all flex flex-col justify-between ${
                        es.isEditing
                          ? 'border-blue bg-blue/5 shadow-inner'
                          : hasPain
                            ? 'bg-salmon/10 border-salmon shadow-lg text-white'
                            : 'border-neutral-300 shadow-lg bg-white/30'
                      }`}
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className={`p-1.5 rounded-md group transition-colors border ${
                              hasPain && !es.isEditing
                                ? 'bg-white/20 border-white/40 hover:bg-white/40'
                                : 'bg-white/50 border-slate-200 hover:bg-dark-blue'
                            }`}
                            title="Editar Exercício"
                          >
                            <PencilIcon
                              className={`w-4.5 h-4.5 transition-colors ${hasPain && !es.isEditing ? 'text-white' : 'text-black group-hover:text-white'}`}
                            />
                          </button>
                          <button
                            onClick={() =>
                            ...
                            className={`p-1.5 rounded-md group transition-colors border ${
                              hasPain && !es.isEditing
                                ? 'bg-white/20 border-white/40 hover:bg-red-600'
                                : 'bg-salmon/10 border-salmon hover:bg-salmon'
                            }`}
                            title="Deletar Exercício"
                          >
                            <TrashIcon
                              className={`w-4.5 h-4.5 transition-colors ${hasPain && !es.isEditing ? 'text-white' : 'text-salmon group-hover:text-white'}`}
                            />
                          </button>
                        </div>
                      </div>
                      ...
                      {es.isEditing ? (
                        <div className="flex flex-col gap-2 mt-4">
                          <div className="grid grid-cols-2 gap-2">
                            <div className="flex flex-col">
                              <label className="text-[10px] uppercase font-bold text-black ml-1">
                                Séries
                              </label>
                              <input
                                type="number"
                                className="border border-black/40 rounded px-2 py-1 text-sm bg-white text-black"
                                value={es.serie}
                                onChange={(e) =>
                                  updateExerciseSessionLocal(
                          ...
                          <button
                            ...
                            className="w-full bg-black text-white text-xs font-bold py-2 rounded hover:opacity-60 transition-colors shadow-sm"
                          >
                            Salvar alterações
                          </button>
                        </div>
                        ...
                        <div>
                          <p
                            className={`${hasPain ? 'text-white/90' : 'text-black/60'} text-sm`}
                          >
                            Séries:{' '}
                            <span
                              className={`font-medium ${hasPain ? 'text-white' : 'text-black'}`}
                            >
                              {es.serie} x {es.repetitions}
                            </span>
                          </p>
                          {hasPain && (
                            <p className="mt-2.5 text-xs text-black font-bold uppercase">
                              Paciente sentiu dor fazendo este exercício!
                            </p>
                          )}
                        </div>
                      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
              <div className="md:col-span-7">
                <div className="bg-white/30 h-fit px-4 pt-4 pb-8 rounded-xl border border-neutral-300 shadow-lg">
                ...
                <div className="mt-4 p-4 bg-blue/10 border border-blue-200 rounded-lg shadow-lg">
                  <div className="flex items-center gap-2 my-1">
                    {appointmentOnSelectedDate && (
                      <span className="bg-dark-blue text-white text-sm font-bold px-2 py-0.5 rounded">
                        CONSULTA
                      </span>
                    )}
                    <span className="text-dark-blue font-bold">
                      {appointmentOnSelectedDate?.time ||
                        'Não há consultas marcadas nesta data.'}
                    </span>
                  </div>
                  {appointmentOnSelectedDate && (
                    <p className="text-sm text-black/80">
                      {appointmentOnSelectedDate.description ||
                        'Sem observações.'}
                    </p>
                  )}
                </div>
              ...
              <div className="md:col-span-5">
                <form
                  ...
                  className="bg-white/30 h-full p-4 rounded-xl border border-neutral-300 shadow-lg space-y-4"
                >
                  <h3 className="text-lg font-semibold text-black mb-4">
                    Novo Agendamento
                  </h3>
                  <div>
                    <label className="text-sm text-black/60 uppercase font-bold">
                      Horário
                    </label>
                    <input
                      type="time"
                  ...
                  <button
                    ...
                    className={`w-full py-4 rounded-lg font-bold text-white transition-all shadow-md ${isSavingAppointment ? 'bg-black/40' : 'bg-black hover:bg-black/80'}`}
                  >
                    {isSavingAppointment
                      ? 'Agendando...'
